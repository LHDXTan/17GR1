	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (opr.equals("update")) { // 更新主题
			...
			if (t == null) {
				if (topicsBiz.updateTopic(topic) > 0) {
					out.print("<script type='text/javascript'>"
							...
							+ "/TopicServlet?opr=list';</script>");
				} else {
					out.print("<script type='text/javascript'>"
							+ "alert('更新主题失败，点击确认返回主题列表');" + "location.href='"
							+ contextPath
							+ "/newspages/topic_list.jsp';	</script>");
				}
			} else {
				out.print("<script type='text/javascript'>"
						+ "alert('更新主题失败，此主题已经存在');" + "location.href='"
						+ contextPath + "/newspages/topic_modify.jsp?tid="
						+ tid + "&tname=" + tname + "';	</script>");
			}
			...
			if (topic == null) {
				...
				out.print("<script type='text/javascript'>"
						+ "alert('当前主题创建成功，点击确认返回主题列表！');" + "location.href='"
						+ contextPath + "/TopicServlet?opr=list';"
						+ "</script>");
			} else {
				out.print("<script type='text/javascript'>"
						+ "alert('当前主题已存在，请输入不同的主题！');" + "location.href='"
						+ contextPath + "/newspages/topic_add.jsp';"
						+ "</script>");
			}
			...
			if (result == 1) {
				...
				out.print("<script type='text/javascript'>"
						+ "alert('已经成功删除主题，点击确认返回原来页面！');" + "location.href='"
						+ contextPath + "/TopicServlet?opr=list';"
						+ "</script>");
			} else if (result == -1) {
				out.print("<script type='text/javascript'>"
				...
				out.print("<script type='text/javascript'>"
						+ "alert('该主题下还有文章，不能删除！');" + "location.href='"
						+ contextPath + "/TopicServlet?opr=list';"
						+ "</script>");
			}
		}
