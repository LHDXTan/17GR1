...
<%  List<News> list1 = (List<News>)session.getAttribute("list1");
      ...
      List<News> list5 = null;  
      Page pages=(Page)request.getAttribute("pages");
      String Tid=(String)request.getAttribute("tid");//当前主题 
      if(pages==null||(list1==null&&list2==null&&list3==null&&list4==null&&list5==null)){
      	request.getRequestDispatcher("util/news_control.jsp?opr=listTitle&page_no=1").forward(request, response);
      	return;
      }else{	
      	int page_no=pages.getCurrPageNo();
      	int total_page=pages.getTotalPageCount();	
      	list5 =pages.getNewsList();
%>
  ...
  <div class="main">
    ...
    <div class="content">
      ...
      <ul class="classlist">
	   ...
	   <%}  else if(page_no < total_page && page_no > 1){%>
	   <p align="right"> 当前页数:[<%=page_no%>/<%=total_page  %>]&nbsp;&nbsp; <a href="util/news_control.jsp?opr=topicNew&page_no=1&tid=<%=Tid %>">首页</a><a href="util/news_control.jsp?opr=topicNew&page_no=<%=page_no-1%>&tid=<%=Tid %>">&nbsp;&nbsp;上一页</a><a href="util/news_control.jsp?opr=topicNew&page_no=<%=page_no+1%>&tid=<%=Tid %>">&nbsp;&nbsp;下一页</a> <a href="util/news_control.jsp?opr=topicNew&page_no=<%=total_page%>&tid=<%=Tid %>">&nbsp;&nbsp;末页</a></p>
	   <%}
     	 %> 
