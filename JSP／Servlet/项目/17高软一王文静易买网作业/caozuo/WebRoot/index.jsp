...
function huoqushangpin(id) {
	$.getJSON("product_servler","org=zssp&sjid="+id,function(json){
		var $jjj=$(json);
		$(".menu_bug").empty();
		$(".cate_list").empty();
		$jjj.each(function(i,v){
			$(".cate_list").append("<li>"+"<div class='img'><a href='#'><img src='imges/"+v.fileName"'width='210' height='185'/></a></div>"
					+"<div class='price'>"
	...
	<div class="soubg">
		<div class="sou">
			...
			<span class="fr">
			 <span class="fl"> <c:if
						test="${user==null}">
        	你好<a href="login.jsp">登录</a>&nbsp; <a href="Regist.jsp"
							style="color:#ff4e00;">免费注册</a>&nbsp;|&nbsp;<a href="#">我的订单</a>&nbsp;|</span>
				</c:if> <c:if test="${user!=null}">
        	你好${user}&nbsp<a href="loginServlet?org=remove"
						style="color:#ff4e00;">注銷</a>&nbsp;|&nbsp;
						<a href="product.jsp">后台管理</a>
						...
						<%
							product_categoryDaoServiceimp	p=new product_categoryDaoServiceimp();
													List<product_categoryVo> queryAllProductCategoryList = p.queryAllProductCategoryList();
													  int i=1;
													for(product_categoryVo yiji:queryAllProductCategoryList){
						%>
						<li><div class="fj">
								<span class="n_img"> <img src="images/nav<%=i%>.png" /></span>
								<span class="fl"><%=yiji.getProduct_category().getName()%></span>
							</div>
							<div class="zj">
								<div class="zj_l">
									<%
										i++;
																                	  List<product_categoryVo> queryAllProductCategoryList2 =yiji.getProductDaXiangzi();
																                	   for(product_categoryVo erji:queryAllProductCategoryList2){
									%>
									<div class="zj_l_c">
										<h2><%=erji.getProduct_category().getName()%></h2>
										<%
											List<product_categoryVo> queryAllProductCategoryList3=erji.getProductDaXiangzi();
																		                		   for(product_categoryVo sanji:queryAllProductCategoryList3){
										%>
										<a href="#"
