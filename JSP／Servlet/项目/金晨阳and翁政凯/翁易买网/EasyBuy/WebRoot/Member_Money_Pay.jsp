      ...
      <table border="0" class="order_tab" style="width:930px; text-align:center; margin-bottom:30px;" cellspacing="0" cellpadding="0">
        ...
        <script type="text/javascript">
	         $(document).ready(function(){
	          ...
	          function detele(id){
	        	  var a = confirm("确认删除？");
	        	  if(a==true){
		        	 location.href="OrderServlet?action=detelefenlei&id="+id;
	        	  }
	          }
          </script>
        <c:forEach items="${productCategoryList}" var="temp">
          <tr>
            ...
            <td>
	            <c:if test="${empty temp.parentld && temp.parentld==0}">
	            	无
	            </c:if>
	            <c:if test="${not empty temp.parentld}">
	            	${temp.parentld}
	            </c:if>
            </td>
            <td><a href="" onclick="detele(${temp.id})">删除</a></td>
          </tr>
        </c:forEach>
        </tbody>
      </table>
