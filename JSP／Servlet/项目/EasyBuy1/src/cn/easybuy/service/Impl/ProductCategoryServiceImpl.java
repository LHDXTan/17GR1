...
public class ProductCategoryServiceImpl implements ProductCategoryService {
	...
	public List<ProductCategoryVo> queryAllProductCategoryList() {
		...
		for (ProductCategory product1Category : productCategory1List) {
			...
			// 根据一级分类查询二级分类
			List<ProductCategory> productCategory2List = getProductCategories(product1Category
					.getId());
			for (ProductCategory productCategory2 : productCategory2List) {
				...
				productCategoryVo2
						.setProductCategoryVoList(productCategoryVo2ChildList);
				// 根据二级分类查询三级分类的列表
				List<ProductCategory> productCategory3List = getProductCategories(productCategory2
						.getId());
			...
			productCategoryVo
					.setProductCategoryVoList(productCategoryVo1ChildList);
	...
	private List<ProductCategory> getProductCategories(Integer parentId) {// 根据父ID查询所有子商品分类
		...
		try {
			...
			productCategoryList = productCategoryDao
