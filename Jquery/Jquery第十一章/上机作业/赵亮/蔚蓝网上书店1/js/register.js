$(function(){
	$("#registerBtn").click(function (){
		...
		if(e.validity.valueMissing==true){
			e.setCustomValidity("邮箱不能为空");
		}else if(e.validity.typeMismatch==true){
			e.setCustomValidity("邮箱格式不正确，如web@ba.cn")
		}else{
			e.setCustomValidity("");
		}
		//昵称
		...
		if(name.validity.valueMissing==true){
			name.setCustomValidity("请输入你的昵称");
		...
		if(pwd.validity.valueMissing==true){
			pwd.setCustomValidity("请输入你的密码");
		}else if(pwd.validity.patternMismatch==true){
			pwd.setCustomValidity("密码必须是6~20位的英文和数字");
		}else{
			pwd.setCustomValidity("");
		}
		var xin=document.getElementById("repwd");
		 if(xin.value!=pwd.value){
			xin.setCustomValidity("两次密码不一致");
		}else{
			xin.setCustomValidity("");
		}
		//地区
 return  true;
   ...
   /*  $("#province").append(function(){
    ...
    $("#province").change(function(){
    	...
    	for (n in citys){
    		html+="<option value="+citys[n]+">"+citys[n]+"</option>";
